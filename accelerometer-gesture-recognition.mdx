...
Buttons wear out, touchscreens add cost and complexity, and neither works well when the user is wearing gloves or the device is sealed inside an enclosure. An accelerometer costs under a dollar, has no moving parts, and can distinguish dozens of distinct gestures with a small neural network running entirely on the MCU. This lesson walks through the full pipeline: collecting labeled IMU data from an MPU6050, training a gesture classifier in TensorFlow, deploying TFLite Micro models on both the RPi Pico and STM32, and comparing inference latency, memory footprint, and power consumption across both platforms. #GestureRecognition #TinyML #EmbeddedML

<PodcastEmbed src="https://open.spotify.com/episode/2nqmzlOxX6suAfDOQZc4Ie?si=7JJ9Y1DvT1aH-xh5ZRQuyg" />
...
:::tip[Instrument it]
Gesture and motion models live or die by where the accelerometer sits. Mount it rigidly to the thing that actually moves, aligned so the axis you care about sees the motion cleanly, and keep that orientation identical between training and deployment, since a model trained with the chip one way fails when it is rotated. The same discipline turns this from a gesture demo into machine-motion monitoring: an IMU on a pump or a door learning what normal looks like, and [flagging what is not](https://siliconwit.io).
:::
