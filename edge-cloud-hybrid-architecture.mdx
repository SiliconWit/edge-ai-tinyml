...
```mermaid
flowchart TD
    A[Sensor Data] --> B[Edge Model\n12 KB on ESP32]
    B --> C{MSE threshold?}
    C -- MSE below low --> D[NORMAL\nlocal decision]
    C -- MSE above high --> E[ANOMALY\nlocal decision]
    C -- MSE in between --> F[UNCERTAIN\nescalate to cloud]
    F --> G[Cloud Model\n200 KB, definitive answer]
    G --> H[Accumulate escalated data]
    H --> I[Retrain edge model]
    I --> J[OTA push new .tflite]
    J --> B
    D --> K[MQTT Dashboard]
    E --> K
    G --> K
...
:::tip[Instrument it]
This is the architecture question every real monitor faces: what does the device decide on its own, and what does it send up? Detect at the edge and you ship an event ("bearing anomaly at 14:02") instead of a continuous vibration stream, which saves bandwidth, battery, and cloud cost while still letting a [dashboard and alerts](https://siliconwit.io) act on it. Keep raw data local for the rare deep look, and push the conclusions and the exceptions. That split is what makes monitoring scale past a handful of devices.
:::
