...
| Dashboard | Grafana or SiliconWit.io (covered in IoT Systems course) |
...
void reload_tflite_interpreter(const uint8_t *model_data, size_t model_size)
{
    ...
    // Add only the ops your autoencoder uses
    ...
    printf("Interpreter reloaded with new model (%d bytes)\n", model_size);
}
```
