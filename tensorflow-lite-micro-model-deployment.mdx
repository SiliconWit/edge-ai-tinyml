...
:::tip[Instrument it]
Deployment is the step that moves the detector from your laptop to the device that actually touches the machine. The model now runs on the [MCU wired to the sensor](/education/sensor-actuator-interfacing-stm32/), reading the same signal it trained on and deciding in milliseconds whether something is wrong. Match the input scaling and sample rate to training exactly, and the device becomes a self-contained monitor that [reports a verdict](/education/iot-systems/), not a data dump, on whatever it is bolted to.
:::
