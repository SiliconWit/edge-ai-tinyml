...
In Lesson 2 you used Edge Impulse to handle model conversion and deployment automatically. That convenience comes at the cost of understanding. In this lesson you will work directly with the TensorFlow Lite for Microcontrollers (TFLM) runtime. You will train a gesture classifier locally in TensorFlow, walk through each stage of model conversion, and deploy the same model on both an ESP32 and an STM32. By porting the model across two different architectures, you will understand exactly how the TFLM interpreter, tensor arena, and op resolver work, and where platform-specific code lives. #TFLiteMicro #ESP32 #STM32
...
extern "C" void app_main(void) {
    ...
    ESP_LOGI(TAG, "Input: scale=%.6f zp=%ld", input_scale, input_zp);
    ESP_LOGI(TAG, "Output: scale=%.6f zp=%ld", output_scale, output_zp);
    ...
    while (1) {
        ...
        for (int i = 0; i < NUM_FEATURES; i++) {
            ...
            input->data.int8[i] = (int8_t)(normalized / input_scale + input_zp);
        }
...
int main(void) {
    ...
    while (1) {
        ...
        for (int i = 0; i < NUM_FEATURES; i++) {
            ...
            input->data.int8[i] = (int8_t)(norm / input_scale + input_zp);
        }
