...
**Lesson 9: Edge-Cloud Hybrid Architectures.** The next lesson ties edge inference to cloud infrastructure. You will build a system where the ESP32 runs local classification, escalates uncertain results to the cloud for a larger model, and receives updated models via OTA. If you completed the [IoT Systems course](/education/iot-systems/), you already have the MQTT, dashboard, and REST API skills that Lesson 9 builds on.
...
:::tip[Instrument it]
A camera is a sensor too, and on a machine it reads what no other sensor can: an analog gauge needle, a fill level, a warning lamp, whether the right part is in the fixture. Mount it with steady, consistent lighting and a fixed view (changing light is the usual reason these models drift), classify on the device so only the result leaves and not a video stream, and you have visual monitoring that can [report a state or raise a flag](https://siliconwit.io).
:::
