...
<PodcastEmbed src="https://open.spotify.com/episode/0helatWKSTmQkwtEmUGfcl?si=zONVcLp-Siaa8F09Ek9UmA" />
...
```mermaid
flowchart TD
    A[Float32 model] --> B{Small enough?\nunder 50 KB, under 10 ms}
    B -- Yes --> C[Apply PTQ and deploy]
    B -- No --> D{PTQ accuracy OK?\nless than 2% drop}
    D -- Yes --> C
    D -- No --> E[Apply QAT\nfine-tune 20-30 epochs]
    E --> F{Still too large\nfor flash?}
    F -- Yes --> G[Reduce architecture\nfewer layers/filters, retrain]
    G --> B
    F -- No --> H{Need OTA\ncompression?}
    H -- Yes --> I[Prune before quantize\nthen gzip/zstd]
    H -- No --> J[Deploy]
```
...
:::tip[Instrument it]
Quantization is what lets a real monitor exist at all. A model that runs in int8 in a few kilobytes is one you can put on a 3-dollar microcontroller bolted next to the sensor, running on a coin cell, instead of a Raspberry Pi that needs mains power. That is the whole promise of monitoring at the edge: cheap enough to deploy on every machine, small enough to sip power, and private enough that only the [detection leaves the device](/education/iot-systems/) for [siliconwit.io](https://siliconwit.io), never the raw stream.
:::
