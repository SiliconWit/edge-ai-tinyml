...
<PodcastEmbed src="https://open.spotify.com/episode/7iMHvUA2R84z83DVNuBxZ3?si=4El8yOw9SwuDoMZQhDHc1Q" />
...
```mermaid
flowchart TD
    A[Mic captures\n16 kHz audio] --> B[Frame into\n32 ms windows]
    B --> C[Extract MFCCs\n13 coefficients/frame]
    C --> D[Build spectrogram\n62 frames x 13]
    D --> E[CNN classifies\nyes / no / unknown]
    E --> F{Confidence\nabove 0.85?}
    F -- 2 consecutive --> G[DETECTED]
    F -- No --> A
```
...
:::tip[Instrument it]
The same wake-word pipeline is how a machine listens to itself. A failing bearing, a cavitating pump, a leaking valve, and an arcing contact each have an acoustic signature, and a tiny model on a microphone can learn to spot it just like a keyword. Mount the [microphone](/education/sensor-actuator-interfacing-stm32/) close to the source and shielded from wind and handling noise, classify on-device so only the detection leaves, and the machine can [call out its own faults](https://siliconwit.io) by the sound it makes.
:::
